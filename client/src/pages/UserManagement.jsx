...
const UserManagement = () => {
  ...
  const handleEditUser = async (editedUser) => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/users/${editedUser.id}`,
        editedUser
      );
  ...
  const sortedUsersWithAccess = [...usersWithAccess].sort(
    (a, b) => a.id - b.id
  );
  const sortedUsersWithoutAccess = [...usersWithoutAccess].sort(
    (a, b) => a.id - b.id
  );
  ...
  const filteredUsers = (
    currentTab === "ALL"
      ...
      ? sortedUsersWithAccess
      : sortedUsersWithoutAccess
  ).filter((user) => {
    const matchesSearch =
