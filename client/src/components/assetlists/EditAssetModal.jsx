...
const EditAssetModal = ({
  ...
  useEffect(() => {
    if (asset) {
      ...
      const totalQuantity =
        parseInt(asset.quantity) + parseInt(asset.quantity_for_borrowing || 0);
  ...
  const handleChange = async (field, value) => {
    if (
      field === "type" &&
      value === "Consumable" &&
      editedAsset.type === "Non-Consumable"
    ) {
    ...
    setEditedAsset((prev) => {
      ...
      if (field === "quantity" || field === "cost") {
        ...
        const totalQuantity =
          parseInt(newQuantity) + parseInt(prev.quantity_for_borrowing || 0);
  ...
  const calculateTotalCost = (quantity, cost) => {
    if (quantity && cost) {
      // Include quantity_for_borrowing in total cost calculation
      const totalQuantity =
        parseInt(quantity) + parseInt(editedAsset?.quantity_for_borrowing || 0);
  ...
  const handleSaveAsset = async () => {
    ...
    if (editedAsset) {
      try {
        ...
        if (
          editedAsset.productCode !== asset.productCode &&
          ...
          const checkResponse = await axios.get(
            ...
            }/api/assets/check-product-code/${encodeURIComponent(
              editedAsset.productCode
            )}`
          );
        ...
        if (
          editedAsset.serialNumber !== asset.serialNumber &&
          ...
          const checkSerialResponse = await axios.get(
            `${
              process.env.REACT_APP_API_URL
            }/api/assets/check-serial-number/${encodeURIComponent(
              editedAsset.serialNumber
            )}`
          );
        ...
        const updatedAsset = {
          ...
          quantity_for_borrowing: editedAsset.is_active
            ? parseInt(quantityForBorrowing, 10)
            : 0,
        ...
        const changedFields = Object.keys(updatedAsset).reduce((acc, key) => {
          ...
          if (
            ...
            acc.hasOwnProperty("quantity_for_borrowing")
          ) {
            return acc;
          }
          if (key === "totalCost") {
            // Only include totalCost if it has actually changed
            const oldTotalCost = parseFloat(
              (asset.cost * asset.quantity).toFixed(2)
            );
            ...
            JSON.stringify(updatedAsset[key]) !== JSON.stringify(asset[key])
          ) {
            acc[key] = {
              oldValue: asset[key],
        ...
        if (Object.keys(changedFields).length > 0) {
          ...
          const response = await axios.put(
            `${process.env.REACT_APP_API_URL}/api/assets/update/${updatedAsset.asset_id}`,
            updatedAsset
          );
          ...
          await axios.post(
            `${process.env.REACT_APP_API_URL}/api/asset-activity-logs`,
            {
  ...
  const handleTypeChange = async (value) => {
    ...
    if (
      ...
      value === "Consumable" &&
      asset.quantity_for_borrowing > 0
    ) {
      setNotification({
        type: "error",
        message:
          "Cannot change to Consumable while asset has borrowing quantity. Please deactivate borrowing first.",
      });
      return;
    }
  ...
  const checkAssetStatus = async (assetId) => {
    try {
      ...
      return {
        canChangeType:
          !hasActiveRepairs && !hasActiveIssues && !hasActiveBorrowRequests,
        message: hasActiveRepairs
          ? "Cannot change to Consumable: Asset has active repair records"
          : hasActiveIssues
          ? "Cannot change to Consumable: Asset has active issue records"
          : hasActiveBorrowRequests
          ? "Cannot change to Consumable: Asset has pending borrow requests"
          : "",
      };
    } catch (error) {
            ...
            <form className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputField
                  ...
                  onChange={(e) =>
                    handleChange("cost", e.target.value.replace(/[^0-9.]/g, ""))
                  }
                  prefix="₱"
              ...
              {editedAsset.is_active && (
                <InputField
                  ...
                  onChange={(e) =>
                    handleQuantityForBorrowingChange(
